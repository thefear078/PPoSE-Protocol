...
fn alice_bob_two_hops() {
    ...
    let bob_hops = vec![hop2, hop1];
    let alice_hops = vec![hop1, hop2];

    let bob_thread = thread::spawn(move || {
        let mut bob = UdpSession::accept_responder_onion(bob_sock, &bob_id, bob_hops, alice_addr)
            .expect("bob hs");
        let msg = bob.recv(Duration::from_secs(4)).expect("bob recv");
        assert_eq!(msg, b"onion-hi");
        bob.send(b"onion-ok").expect("bob send");
    });
    ...
    .expect("alice hs");
    alice.send(b"onion-hi").expect("alice send");
    let reply = alice.recv(Duration::from_secs(4)).expect("alice recv");
    assert_eq!(reply, b"onion-ok");
